...
import { CalendarRange, LayoutDashboard, Menu, Stethoscope, Users, X } from "lucide-react";
...
const navItems = [
  ...
  { to: "/doctor/appointments", label: "Appointments", icon: CalendarRange },
];
    ...
    <>
      ...
      >
        <div className="flex items-center gap-3">
          <div className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-tr from-emerald-600 to-teal-400 shadow-lg shadow-emerald-500/30 transition-transform duration-200 group-hover:scale-105">
            <Stethoscope className="size-4 text-white" />
          </div>
          <div className="min-w-0">
            <div className="text-[9px] font-bold uppercase tracking-widest text-emerald-400">{clinicName}</div>
            <div className="truncate text-sm font-extrabold tracking-tight text-white">{doctorName}</div>
          </div>
        </div>
      </Link>
              ...
              <>
                <item.icon
                  className={classNames(
                    ...
                    isActive ? "scale-110 text-white" : "text-slate-500 group-hover:text-emerald-400",
                  )}
                />
...
export function DoctorLayout({ children, doctorName }: PropsWithChildren<{ doctorName: string }>) {
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <header className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3 shadow-sm lg:hidden">
          ...
          <div className="flex min-w-0 items-center gap-2">
            <div className="flex size-7 shrink-0 items-center justify-center rounded-lg bg-gradient-to-tr from-emerald-600 to-teal-400">
              <Stethoscope className="size-3.5 text-white" />
            </div>
            <span className="truncate text-sm font-extrabold text-slate-900">{doctorName}</span>
          </div>
